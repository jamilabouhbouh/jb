...
print('running with load LSQ as {load_lsq}, fix S0 as {fixS0}, segmented as {segmented}\n'.format(load_lsq=load_lsq,fixS0=arg.fixS0, segmented=segmented))
print('running with load_nn as {load_nn}, network as {nn}, optimisr: {optim}, lr={lr}\n'.format(load_nn=load_nn, nn=arg.run_net, optim=arg.optim,lr=arg.lr))
...
if dolsq:
    if not load_lsq:
        if segmented:
            ...
            paramslsq = fit_least_squares_array(bvalues, datatot,fixS0=arg.fixS0)
            paramslsq2 = fit_least_squares_array(bvalues2, datatot2,fixS0=arg.fixS0)
            if arg.fixS0:
...
if not load_nn:
    ...
    torch.save(net2, 'network2_{nn}.pt'.format(nn=arg.run_net))

else:
    net=torch.load('network_{nn}.pt'.format(nn=arg.run_net))
    net2=torch.load('network2_{nn}.pt'.format(nn=arg.run_net))
