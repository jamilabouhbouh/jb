...
if not lsq_only:
    for run_net in ['tiny']:
        ...
        for ii in range(5):
            ...
            names = ['geof_NN_{ii}_{net}_2.nii'.format(ii=ii,net=run_net),'Dp_NN_{ii}_{net}_2.nii'.format(ii=ii,net=run_net), 'D_NN_{ii}_{net}_2.nii'.format(ii=ii,net=run_net), 'f_NN_{ii}_{net}_2.nii'.format(ii=ii,net=run_net), 'S0_NN_{ii}_{net}_2.nii'.format(ii=ii,net=run_net)]
            for k in range(len(names)):
                ...
                nib.save(nib.Nifti1Image(img * multiple[k], data.affine, data.header),
                         'C:/Users/ochampion/Documents/Data/ML_IVIM/{name}.nii.gz'.format(name=names[k]))
